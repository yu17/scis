		...
		(cond
			...
			(else (EL_SL_get expr env (lambda (v)
				(if (equal? v '__SCIS_THIS)
					(EL_Sdump env 'this (lambda (e de) (return_v e)))
					(return_v v))
			))))
	))
						...
						(intpn_funcall_arglist (car f) (cdr stmt) e_func clis c-return c-break c-continue c-throw (lambda (e2)
							(intpn_dot_func_transfer_toplayer e e2 (lambda (e3)
								(intpn_stmt_auto (cadr f) e3 clis
									(lambda (e4 v) (EL_popLayer e4 (lambda (e5) (intpn_dot_routine_exit (cdar stmt) e5 de clis (lambda (e6 cl) 
										(if (null? e6)
											(return_e env)
											(return_e e6)))
									))))
									(lambda (e4) (EL_popLayer e4 (lambda (e5) (intpn_dot_routine_exit (cdar stmt) e5 de clis (lambda (e6 cl) 
										(if (null? e6)
											(c-break env)
											(c-break e6)))
									))))
									...
									(lambda (e4 err) (EL_popLayer e4 (lambda (e5) (intpn_dot_routine_exit (cdar stmt) e5 de clis (lambda (e6 cl) 
										(if (null? e6)
											(c-throw env err)
											(c-throw e6 err)))
									))))
...
(define intpn_dot_routine_enter
	(lambda (stmt env clis c-return c-break c-continue c-throw return_e_de)
		(cond
			...
			((equal? (car stmt) 'this) (EL_Sdump env 'this return_e_de))
...
(define intpn_dot_routine_exit
	(lambda (stmt env denv clis return_e_cl)
		(cond
			...
			((or
				(equal? (car stmt) 'this)
				(equal? (car stmt) 'super)) (EL_Srestore env denv (lambda (e) (return_e_cl e clis))))
			((EL_SL_check (car stmt) denv (lambda (b) b)) (EL_SL_set (list (car stmt) env) denv (lambda (ve)
				(return_e_cl ve clis))
			))
